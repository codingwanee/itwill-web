...
public class SuggestDAO extends JdbcDAO {
	...
	//키워드를 전달받아 키워드가 포함된 제시어 관련 정보를 SUGGEST 테이블에서 검색하여 반환하는 메소드
	public List<SuggestDTO> getSuggestList(String keyword) {
		Connection con=null;
		...
		try {
			...
			System.out.println("[에러]getSuggestList() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return suggestList;
