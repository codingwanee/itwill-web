...
public class AjaxCommentDAO extends JdbcDAO {
	...
	//댓글정보를 전달받아 AJAX_COMMENT 테이블에 저장하고 저장행의 갯수를 반환하는 메소드
	public int addAjaxComment(AjaxCommentDTO ajaxComment) {
		Connection con=null;
		...
		try {
			...
			System.out.println("[에러]addAjaxComment() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt);
		}
		return rows;
	...
	//댓글정보를 전달받아 AJAX_COMMENT 테이블에 저장된 댓글을 변경하고 변경행의 갯수를 반환하는 메소드
	public int modifyAjaxComment(AjaxCommentDTO ajaxComment) {
		Connection con=null;
		PreparedStatement pstmt=null;
		int rows=0;
		try {
			...
			System.out.println("[에러]modifyAjaxComment() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt);
		}
		return rows;
	...
	//댓글번호를 전달받아 AJAX_COMMENT 테이블에 저장된 댓글을 삭제하고 삭제행의 갯수를 반환하는 메소드
	public int removeAjaxComment(int num) {
		Connection con=null;
		PreparedStatement pstmt=null;
		int rows=0;
		try {
			...
			System.out.println("[에러]removeAjaxComment() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt);
		}
		return rows;
	...
	//댓글번호를 전달받아 AJAX_COMMENT 테이블에 저장된 댓글을 검색하여 반환하는 메소드
	public AjaxCommentDTO getAjaxComment(int num) {
		Connection con=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		...
		try {
			...
			System.out.println("[에러]getAjaxComment() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return ajaxComment;
	...
	//AJAX_COMMENT 테이블에 저장된 모든 댓글을 검색하여 댓글목록을 반환하는 메소드
	public List<AjaxCommentDTO> getAjaxCommentList() {
		Connection con=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		...
		try {
			...
			System.out.println("[에러]getAjaxCommentList() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
