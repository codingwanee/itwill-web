...
public class AjaxMemberDAO extends JdbcDAO {
	...
	//회원정보를 전달받아 AJAX_MEMBER 테이블에 저장되고 저장행의 갯수를 반환하는 메소드
	public int addAjaxMember(AjaxMemberDTO ajaxMember) {
		Connection con=null;
		...
		try {
			...
			System.out.println("[에러]addAjaxMember() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt);
		}
		return rows;
	...
	//아이디를 전달받아 AJAX_MEMBER 테이블에서 해당 아이디의 회원정보를 검색하여 반환하는 메소드
	public AjaxMemberDTO getAjaxMember(String id) {
		Connection con=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		...
		try {
			...
			System.out.println("[에러]getAjaxMember() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return ajaxMember;
	...
	public String getAjaxMemberId(AjaxMemberDTO ajaxMember) {
		Connection con=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		String id=null;
		try {
			...
			System.out.println("[에러]getAjaxMemberId() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return id;
