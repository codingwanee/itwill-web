...
public class MemberDAO extends JdbcDAO {
	...
	// => 아이디,비밀번호,이름,이메일,전화번호,우편번호,기본주소,상세주소는 전달값을 이용하여 저장
	// => 회원가입날짜 : 현재, 마지막 로그인 날짜 : null, 회원상태 : 1
	...
	//아이디를 전달받아 MEMBER 테이블에 저장된 회원정보를 변경하고 변경행의 갯수를 반환하는 메소드
	// => 전달 아이디를 비교하여 마지막 로그인 날짜를 현재로 변경
	public int modifyLastLogin(String id) {
		...
		try {
			...
			System.out.println("[에러]modifyLastLogin() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt);
		}
		return rows;
	...
	//아이디와 상태를 전달받아 MEMBER 테이블에 저장된 회원정보 중 상태를 변경하고 변경행의 갯수를 반환하는 메소드
