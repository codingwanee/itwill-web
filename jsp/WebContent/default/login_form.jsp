...
<%-- => 세션으로 공유된 인증정보를 이용하여 상태 구분 --%>
<%-- 비로그인 상태 : 로그인 정보를 입력받아 처리페이지(login_action.jsp)로 전달 --%>    
<%-- 로그인 상태 : 환영메세지 출력 --%>    
<%
	String message=(String)session.getAttribute("message");
	if(message==null) {//공유 인스턴스(메세지)가 없는 경우
		message="";
	} else {//공유 인스턴스(메세지)가 있는 경우
		//ScopeInstance.removeAttribute(String attributeName)
		// => 공유속성명을 이용하여 공유 인스턴스를 제거하는 메소드
