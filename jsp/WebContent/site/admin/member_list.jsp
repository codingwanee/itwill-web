...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- MEMBER 테이블에 저장된 회원목록을 검색하여 응답하는 JSP 문서 --%>
<%-- => [선택회원삭제]를 클릭한 경우 선택회원의 삭제 처리페이지(member_remove_action.jsp)로 이동 --%>
<%-- => [회원상태]를 변경한 경우 회원상태변경 처리페이지(member_change_action.jsp)로 이동 --%>
...
<table>
	...
	<tr>
		...
		<td class="member_address">[<%=member.getZipcode() %>]
			<%=member.getAddress1() %> <%=member.getAddress2() %></td>
		...
		<td class="member_status">
			<%-- 입력태그의 name 속성값을 아이디(고유값)으로 설정한 이유 --%>
			<%-- => 자바스트립트에서 고유값 전달을 목적으로 설정 --%>
			
			<select class="status" name="<%=member.getId()%>">
				<option value="1" <% if(member.getStatus()==1) { %> selected <% } %>>&nbsp;일반사용자&nbsp;</option>
...
$("#removeBtn").click(function() {
	...
	$("#memberForm").attr("action","<%=request.getContextPath()%>/site/admin/memeber_remove_action.jsp");
	$("#memberForm").submit();
});
...
$(".status").change(function() {
	...
	var status=$(this).val();
	//alert(id+" = "+status);
	location.href="<%=request.getContextPath()%>/site/admin/member_change_action.jsp?id="+id+"&status="+status;
});
</script>
