...
<%-- => [회원가입]를 클릭한 경우 회원가입 처리페이지(member_join_action.jsp)로 입력값을 전달하여 이동  --%>
<%-- => [아이디 중복 검사]를 클릭한 경우 새창에서 아이디 검사페이지(id_check.jsp)로 이동 --%>    
<style type="text/css">
fieldset {
	text-align: left;
	margin: 10px auto;
...
<%-- 템플릿 페이지를 이용하여 사이트를 구현할 경우 
리소스 파일의 경로는 절대경로로 표현하는 것을 권장 --%>
<%-- 컨텍스트 이름은 변경 가능하므로 메소드를 호출하여 반환받아 사용 --%>
<form name="joinForm" id="join" action="<%=request.getContextPath() %>/site/member/member_join_action.jsp" method="post">
...
$("#idCheck").click(function() {
	...
	window.open("<%=request.getContextPath()%>/site/member/id_check.jsp?id="+$("#id").val(),"아이디중복검사","width=400,height=100,left=450,top=250");
});
...
$("#postSearch").click(function() {
	window.open("<%=request.getContextPath()%>/site/member/post_search.jsp","우편번호검색","width=550,height=580,left=350,top=30");
})
</script>
