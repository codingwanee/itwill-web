...
public class GuestDAO extends JdbcDAO {
	...
	//방명록 정보를 전달받아 GUEST 테이블에 저장하고 결과를 반환하는 메소드
	public int addGuest(GuestDTO guest) {
		Connection con=null;
		...
		try {
			...
			pstmt.setString(3, guest.getHomepage());
			pstmt.setString(4, guest.getTitle());
			pstmt.setString(5, guest.getContent());
			
			rows=pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("[에러]addGuest() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt);
		}
		return rows;
	...
	//방명록 글번호를 전달받아 GUEST 테이블의 행을 삭제하고 결과를 반환하는 메소드
	public int removeGuest(int no) {
		Connection con=null;
		PreparedStatement pstmt=null;
		int rows=0;
		try {
			...
			System.out.println("[에러]removeGuest() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt);
		}
		return rows;
	...
	//GUEST 테이블의 모든 행을 검색하여 반환받는 메소드
	public List<GuestDTO> getGuestList() {
		...
		try {
			...
			while(rs.next()) {
				...
				guest.setHomepage(rs.getString("homepage"));
				guest.setTitle(rs.getString("title"));
				guest.setContent(rs.getString("content"));
				guestList.add(guest);
			}
		} catch (SQLException e) {
			System.out.println("[에러]getGuestList() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return guestList;
